...
// bo'lib bilinadi.
{
  const alphas = [...glow.matchAll(/rgba\([^)]*,\s*\.(\d+)\)/g)].map((m) => Number('0.' + m[1]));
...
// ── 6) BO'LIMLAR AJRALIB TURADI, LEKIN CHOK YO'Q ─────────────────────
{
  ...
  checkTrue('6) chekkalarda so‘nadi', band.includes('rgba(255,255,255,0) 0%') && band.includes('rgba(255,255,255,0) 100%'));
  const a = /rgba\(255,255,255,\.(\d+)\)/.exec(band);
  checkTrue('6) farq 2% dan oshmaydi', !!a && Number('0.' + a[1]) <= 0.02);
}
...
// ── 8) KONTRAST BUZILMAGAN ───────────────────────────────────────────
// Eng och nuqta (#16110a) ustida oq matn WCAG AAA (7:1) dan yuqori
// bo'lishi shart. Hisob shu yerda bajariladi — izohdagi raqamga
// ishonib bo'lmaydi, u eskirishi mumkin.
{
  ...
  const lightest = (/radial-gradient\(120% 70% at 50% 0%,(#[0-9a-f]{6})/.exec(css) || [])[1];
  checkTrue('8) eng och nuqta topildi', !!lightest);
  checkTrue('8) oq matn AAA dan yuqori', ratio('#f6efe0', lightest) > 7);
  // Eng xira kulrang matn ham AA (4.5:1) dan past tushmasin.
  checkTrue('8) xira matn AA dan yuqori', ratio('#8a7f68', lightest) > 4.5);
}
