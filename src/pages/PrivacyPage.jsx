...
const CONTENT = {
  uz: {
    ...
    updated: 'Oxirgi yangilanish: 2026-yil 24-sentabr',
    ...
    sections: [
      ...
      { h: 'Saqlash muddati', p: "Ma'lumotlar hisobingiz faol ekan saqlanadi. Hisob o'chirilgach profil va kontent ommadan darhol olib tashlanadi. O'chirilgan post, istoriya, izoh, rasm, video va fayllar ham ommadan darhol olinadi, lekin ularning nusxasi (kim, qachon joylagan va kim o'chirgan) yopiq arxivda saqlanadi — faqat shikoyatlarni ko'rib chiqish, qoidabuzarlik va firibgarlikni tekshirish hamda vakolatli davlat organlarining qonuniy so'rovlariga javob berish uchun. Arxivni hech kim ommaga ko'rmaydi; unga faqat vakolatli adminlar kiradi va har bir qidiruv jurnalga yoziladi. Buyurtma va to'lov yozuvlari buxgalteriya va soliq talablari uchun qonunda belgilangan muddatgacha, xavfsizlik jurnallari 5 yilgacha saqlanadi." },
  ...
  ru: {
    ...
    updated: 'Последнее обновление: 24 сентября 2026',
    ...
    sections: [
      ...
      { h: 'Срок хранения', p: 'Данные хранятся, пока аккаунт активен. После удаления аккаунта профиль и контент сразу убираются из публичного доступа. Удалённые посты, истории, комментарии, фото, видео и файлы тоже сразу исчезают из публичного доступа, но их копия (кто и когда опубликовал и кто удалил) хранится в закрытом архиве — только для рассмотрения жалоб, проверки нарушений и мошенничества и ответа на законные запросы уполномоченных государственных органов. Архив не публикуется; доступ есть только у уполномоченных администраторов, каждый поиск записывается в журнал. Записи заказов и платежей хранятся в течение срока, установленного законом для бухгалтерии и налогов, журналы безопасности — до 5 лет.' },
  ...
  en: {
    ...
    sections: [
      ...
      { h: 'Retention', p: 'Data is kept while your account is active. When the account is deleted, your profile and content are removed from public view immediately. Deleted posts, stories, comments, photos, videos and files also disappear from public view immediately, but a copy (who posted it and when, and who deleted it) is kept in a closed archive — only to review reports, investigate abuse and fraud, and answer lawful requests from authorised state bodies. The archive is never public; only authorised admins can access it and every search is logged. Order and payment records are kept for the period required by accounting and tax law; security logs for up to 5 years.' },
