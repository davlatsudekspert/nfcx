  ...
  return (
    <main className="mx-auto w-full max-w-[1800px] px-6 sm:px-10 lg:px-14 pb-16">
      <section className="pt-14">
        ...
        <h1 className="mt-4 text-3xl font-extrabold tracking-tight">{t('Bildirishnomalar')} {totalCount > 0 && <span className="text-accent">({totalCount})</span>}</h1>
      </section>

      <section className="mt-8 max-w-2xl space-y-3">
        {loading && <div className="text-base-content/45">{t('Yuklanmoqda...')}</div>}

        {!loading && totalCount === 0 && (
          <div className="rounded-2xl border border-dashed border-white/15 p-10 text-center text-base-content/45">{t("Hozircha bildirishnomangiz yo'q.")}</div>
        )}
        ...
        {gifts?.map((g) => (
          <div key={'gift' + g.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-accent/30 bg-accent/5 px-4 py-3 text-sm">
            <span>{'\u{1F381}'} <b className="font-mono">{g.code}</b> — <span className="text-base-content/60">{g.fromEmail}</span> {t('sizga sovg‘a qilmoqchi')}</span>
            <button className="btn btn-accent btn-xs" onClick={() => navigate('/account')}>{t("Ko'rish")}</button>
          </div>
        ))}
        ...
        {auctions?.map((a) => (
          <div key={'auc' + a.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-warning/40 bg-warning/10 px-4 py-3 text-sm">
            <span>{'\u{1F3C6}'} <b className="font-mono">{a.code}</b> {t("auksionida g'olib bo'ldingiz — to'lov kutilmoqda")}</span>
        ...
        {repliedSupport.map((m) => (
          <div key={'sup' + m.id} className="rounded-xl border border-white/10 bg-base-200/50 px-4 py-3 text-sm">
            <div className="text-xs text-base-content/45">{t('{when} murojaatingizga javob keldi', { when: timeAgo(new Date(m.createdAt).getTime()) })}</div>
