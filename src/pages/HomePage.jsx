...
import { parseCode, priceFor, BASE_PRICE, TOTAL_COMBOS } from '../lib/pricing.js';
  ...
  return (
    <main className="wrap">
      <section className="hero">
        <div className="eyebrow"><span className="dot"></span> 1 757 600 tadan <span className="mono">{fmt(TOTAL_COMBOS - catalog.length)}</span> tasi hali bo'sh</div>
        ...
        <div className="stats-row">
          ...
          <div className="stat"><b>{fmt(BASE_PRICE)} so'm</b><span>Minimal narx</span></div>
        </div>
      </section>
        ...
        <div className="panel">
          <div className="checker-row">
            <div className="code-input-group">
              <span className="pfx mono">belgi.uz/</span>
          ...
          {checkResult && (
            <div className="check-result">
              ...
              {!checkResult.bad && checkResult.taken && <>
                <span className="pill taken">Band</span> belgi.uz/{checkResult.code} allaqachon olingan — <a onClick={() => navigate(checkResult.code)} style={{ color: 'var(--teal-bright)', cursor: 'pointer', textDecoration: 'underline' }}>sahifasini ko'rish</a>
              </>}
              {!checkResult.bad && !checkResult.taken && <>
                <span className="pill ok">Bo'sh</span> belgi.uz/{checkResult.code} hozircha bo'sh
              </>}
            </div>
          )}
        </div>
      ...
      <section id="narx">
        ...
        <p className="section-desc">Minimal narx {fmt(BASE_PRICE)} so'm. Narx harf va raqam kombinatsiyasining kamyobligiga qarab oshib boradi — takrorlanuvchi yoki tartibli belgilar qimmatroq.</p>
        <div className="panel">
          <div className="calc-grid">
            <div>
              ...
              <div className="breakdown-row"><span className="k">Minimal narx</span><span className="v">{fmt(BASE_PRICE)} so'm</span></div>
            ...
            <div className="price-box">
              ...
              <button className="btn btn-brass" style={{ marginTop: 18, width: '100%' }} disabled={!parsedCalc || calcTaken} onClick={() => setModalCode(parsedCalc.code)}>
                {!parsedCalc ? 'Avval vizitka kiriting' : calcTaken ? 'Bu vizitka band' : ('Bandlash — ' + fmt(calcInfo.total) + " so'm")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid">
          ...
          {[...catalog].sort((a, b) => b.ts - a.ts).slice(0, 24).map((it) => (
            <a key={it.code} className="card" onClick={() => navigate(it.code)}>
              <div className="code">belgi.uz/{it.code}</div>
      ...
      <section id="savollar">
        ...
        <div className="panel" style={{ padding: '8px 28px' }}>
          {[
            ...
            { q: 'Narx qanday hisoblanadi?', a: 'Minimal narx ' + fmt(BASE_PRICE) + ' so\'m. Bunga harf va raqam kombinatsiyasining kamyobligi bo\'yicha koeffitsient qo\'shiladi.' },
      ...
      {modalCode && (
        <ReserveModal
          code={modalCode}
          price={priceFor(modalCode.slice(0, 3), modalCode.slice(3, 5)).total}
