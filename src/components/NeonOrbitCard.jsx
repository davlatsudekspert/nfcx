...
export default function NeonOrbitCard({ code = 'AAA000', name, finish = 'black' }) {
  ...
  return (
    <div className="relative h-[400px] w-[400px] xl:h-[500px] xl:w-[500px]" aria-hidden="false">
      ...
      <div className="absolute inset-[6%] rounded-full bg-[radial-gradient(circle,var(--accent-a14),transparent_68%)] blur-[6px]"></div>

      <div className="absolute inset-[5%] rounded-full border border-[color:var(--accent-primary)]/30 shadow-[0_0_70px_var(--accent-a22),inset_0_0_55px_var(--accent-a08)]"></div>
      <div className="absolute -inset-[calc(5%-10px)] rounded-full border border-dashed border-[color:var(--accent-primary)]/15"></div>
      <div className="absolute inset-[5%] animate-[spinSlow_16s_linear_infinite] rounded-full bg-[conic-gradient(from_0deg,transparent_0_76%,var(--accent-secondary)_94%,transparent_100%)] [-webkit-mask:radial-gradient(farthest-side,transparent_calc(100%_-_4px),#000_calc(100%_-_3px))] [mask:radial-gradient(farthest-side,transparent_calc(100%_-_4px),#000_calc(100%_-_3px))] [filter:drop-shadow(0_0_8px_var(--accent-a45))]"></div>
      <div className="absolute inset-[5%] animate-[spinSlow_16s_linear_infinite] rounded-full">
        <span className="absolute -top-[5px] left-1/2 ml-[-5px] h-2.5 w-2.5 rounded-full bg-[color:var(--accent-secondary)] shadow-[0_0_14px_3px_var(--accent-a45)]"></span>
      </div>
      <div className="absolute inset-[5%] animate-[spinSlow_26s_linear_infinite_reverse] rounded-full">
        <span className="absolute -top-1 left-1/2 ml-[-4px] h-[7px] w-[7px] rounded-full bg-[color:var(--accent-secondary)] opacity-75 shadow-[0_0_14px_3px_var(--accent-a45)]"></span>
      </div>

      {/* Yorug' zarrachalar — chuqurlik his qildiradi */}
      <span className="absolute left-[18%] top-[12%] h-[5px] w-[5px] rounded-full bg-[color:var(--accent-secondary)]/85 shadow-[0_0_10px_2px_var(--accent-a45)] [animation:floatY_6s_ease-in-out_infinite] [animation-delay:.2s]"></span>
      <span className="absolute left-[10%] top-[64%] h-[3px] w-[3px] rounded-full bg-[color:var(--accent-secondary)]/85 shadow-[0_0_8px_2px_var(--accent-a45)] [animation:floatY_6s_ease-in-out_infinite] [animation-delay:1.4s]"></span>
      <span className="absolute left-[84%] top-[22%] h-[4px] w-[4px] rounded-full bg-[color:var(--accent-secondary)]/85 shadow-[0_0_9px_2px_var(--accent-a45)] [animation:floatY_6s_ease-in-out_infinite] [animation-delay:.9s]"></span>
      <span className="absolute left-[80%] top-[76%] h-[3px] w-[3px] rounded-full bg-[color:var(--accent-secondary)]/85 shadow-[0_0_8px_2px_var(--accent-a45)] [animation:floatY_6s_ease-in-out_infinite] [animation-delay:2.1s]"></span>
